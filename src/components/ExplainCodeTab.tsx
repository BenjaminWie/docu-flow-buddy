...
const ExplainCodeTab = ({
  ...
  const convertedDocs = viewMode === 'business' ? businessExplanations.filter(exp => selectedCategory === 'all' || exp.category === selectedCategory).map(exp => ({
    id: `business-${exp.id}`,
          ...
          <div className="flex items-center justify-between gap-4 mb-6">
            <div className="flex items-center gap-4 flex-1">
              ...
              <div className="relative flex-1">
                ...
                <Input placeholder="Search questions and answers..." value={searchTerm} onChange={e => setSearchTerm(e.target.value)} className="pl-10" />
              </div>
              {viewMode === 'business' && <div className="flex items-center gap-2">
                  <Filter className="w-4 h-4 text-gray-500" />
                  <select value={selectedCategory} onChange={e => setSelectedCategory(e.target.value)} className="border rounded px-3 py-1 text-sm">
          ...
          <div className="grid lg:grid-cols-3 gap-6">
            ...
            <div className="lg:col-span-2 space-y-4">
              ...
              {allQAData.length === 0 ? <Card>
                  ...
                  {allQAData.map(qa => <EnhancedQAItem key={qa.id} qa={qa} onAnswerUpdate={fetchQAData} onChatStart={handleChatStart} />)}
                </div>}
            </div>
            ...
            <div className="lg:col-span-1">
              {showChat ? <EnhancedChatInterface repositoryId={repositoryId} initialQuestion={chatQuestion} onQuestionCreate={handleCreateQA} /> : <Card>
                  <CardContent className="pt-6">
                    <div className="text-center py-8 text-gray-500">
                      <MessageSquare className="w-12 h-12 mx-auto mb-4 opacity-50" />
                      <p className="mb-4">Ask your Docu Buddy about:</p>
                      <ul className="text-sm text-left space-y-1 mb-6">
                        <li>• How to set up your development environment?</li>
                        <li>• What are the most important functions I should know by heart?</li>
                        <li>• Help me understand the architecture?</li>
                        <li>• What are the Business benefits and key usp's of this code?</li>
                        <li>• What are the safety standards in the code?</li>
                      </ul>
                      <Button onClick={() => setShowChat(true)}>
